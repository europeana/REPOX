...
public class UnimarcUtil {
     ...
     * @return the 105$a 4-7 or Etiq.reg./6 of the record, null if the record contains none
     */
    public static String getForm(Record r){
    	String dat=r.getSingleFieldValue(105,'a');
    	if(dat!=null && dat.length()>7)
    		return dat.substring(4,8);
    	if (r.getLeader()!=null && r.getLeader().length()>6)
    		return r.getLeader().substring(6,7);
    	return null;
    } 
