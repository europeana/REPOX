 ...
 */
public class RepoxContextUtilEuropeana implements RepoxContextUtil {
    private static final Logger          log                               = Logger.getLogger(RepoxContextUtilEuropeana.class);
    ...
    private static RepoxManagerEuropeana repoxManager;

    public RepoxManagerEuropeana getRepoxManager() {
        try {
            if (repoxManager == null) {
                Properties configurationProperties = PropertyUtil.loadCorrectedConfiguration(CONFIG_FILE);
                RepoxConfigurationEuropeana configuration = new RepoxConfigurationEuropeana(configurationProperties);
                ...
                repoxManager = new RepoxManagerEuropeana(configuration, DATA_PROVIDERS_FILENAME, STATISTICS_FILENAME, RECORD_COUNTS_FILENAME, SCHEDULED_TASKS_FILENAME, RECOVERABLE_TASKS_FILENAME, METADATA_TRANSFORMATIONS_FILENAME, OLD_TASKS_FILENAME, EXTERNAL_SERVICES_FILENAME, METADATA_SCHEMAS_FILENAME, TAGS_FILENAME);
            }
    ...
    public RepoxManagerEuropeana getRepoxManagerTest() {
        try {
            Properties configurationProperties = PropertyUtil.loadCorrectedConfiguration(CONFIG_FILE);
            RepoxConfigurationEuropeana configuration = new RepoxConfigurationEuropeana(configurationProperties);
