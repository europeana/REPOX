 ...
 */
package pt.utl.ist.repox.util.date;
...
public class DateUtil {
    ...
    static public String todayDate() {
        ...
        return String.valueOf(now.get(Calendar.DAY_OF_MONTH)) + "-" + String.valueOf(now.get(Calendar.MONTH) + 1) + "-" + String.valueOf(now.get(Calendar.YEAR));
    }
    ...
    public static String getSeason(Date d) {
        ...
        if (m <= 3) return "Inverno";
        if (m <= 6) return "Primavera";
        if (m <= 9) return "Verão";
        if (m <= 12) return "Outono";
        return "";
    }
    ...
    public static String getDayOfWeek(Date d) {
        ...
        if (ret == Calendar.MONDAY) return "Segunda-feira";
        if (ret == Calendar.TUESDAY) return "Terça-feira";
        if (ret == Calendar.WEDNESDAY) return "Quarta-feira";
        if (ret == Calendar.THURSDAY) return "Quinta-feira";
        if (ret == Calendar.FRIDAY) return "Sexta-feira";
        if (ret == Calendar.SATURDAY) return "Sábado";
        if (ret == Calendar.SUNDAY) return "Domingo";
    ...
    public static String getDayOfWeekAbreviated(Date d) {
        ...
        cal.setTime(d);
        int ret = cal.get(Calendar.DAY_OF_WEEK);
        if (ret == Calendar.MONDAY) return "Seg";
        if (ret == Calendar.TUESDAY) return "Ter";
        if (ret == Calendar.WEDNESDAY) return "Qua";
        if (ret == Calendar.THURSDAY) return "Qui";
        if (ret == Calendar.FRIDAY) return "Sex";
        if (ret == Calendar.SATURDAY) return "Sab";
        if (ret == Calendar.SUNDAY) return "Dom";
        return "";
    }
     ...
     * @param date
     * @param format
     * @return boolean indicating if date is valid on the format
     */
