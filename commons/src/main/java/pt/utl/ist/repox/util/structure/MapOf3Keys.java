...
public class MapOf3Keys<K1, K2, K3, V> implements Serializable {
    ...
    public void remove(K1 key, K2 key2, K3 key3) {
        ...
        if (recs == null) return;
        Map<K3, V> recs2 = recs.get(key2);
        if (recs2 == null) return;
        recs2.remove(key3);
        if (recs2.size() == 0) recs.remove(key2);
     ...
     * @param key2
     * @param key3
     * @return V get the item with the specified keys
     */
     ...
     * @param key
     * @param key2
     * @return get the map with the two specified keys
     */
     ...
     * @param key2
     * @param key3
     * @return boolean if the value exists with the specified keys
     */
    public boolean containsKey(K1 key, K2 key2, K3 key3) {
        Map<K2, Map<K3, V>> midMap = get(key);
        if (midMap == null) return false;
        Map<K3, V> midMap2 = midMap.get(key2);
