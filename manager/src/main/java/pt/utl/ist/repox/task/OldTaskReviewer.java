...
public class OldTaskReviewer {
        ...
        try {
            ...
            for (String logFileName : dataSource.getLogFilenames()) {
                ...
                if (!matchFound && dataSource.getStatus() != DataSource.StatusDS.RUNNING) {
                    ...
                    OldTask oldTask = new OldTask(dataSource, oldTaskId, logFileName, "incrementalIngest", statusAndRecords.getStatus(), "0", "3", "300", convertDateToString(getLogFileDate(dataSetId, logFileName)), statusAndRecords.getRecords());
    ...
    private Date getLogFileDate(String dataSetId, String logFileName) throws StringIndexOutOfBoundsException, ArrayIndexOutOfBoundsException {
        ...
        // Parse Date
        String finalDate = dateAttached.substring(0, 4) + "-" + dateAttached.substring(4, 6) + "-" + dateAttached.substring(6, 8);

        // Parse Time
        String finalTime = timeAttached.substring(0, 2) + ":" + timeAttached.substring(2, 4);
    ...
    private StatusAndRecords getStatusAndRecordsFromLogFile(String dataSetId, String logFileName) {
        File logFile = new File(ConfigSingleton.getRepoxContextUtil().getRepoxManager().getConfiguration().getRepositoryPath() + File.separator + dataSetId + File.separator + "logs" + File.separator + logFileName);
