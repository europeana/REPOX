...
public class ExternalNoMonitorServiceThread extends Thread {
    ...
    public ExternalServiceStates.ServiceExitState runExternalService() {
        try {
            writeOutputMessage("Running External No Monitor Service with ID " + externalRestService.getId() + " with FINAL COMMAND = " + finalUri, logFile);
    ...
    private void buildFinalUri() {
        String uri = externalRestService.getUri() + "?" + externalRestService.getServiceParameters().get(0).getName() + "=" + externalRestService.getServiceParameters().get(0).getSemantics();

        for (int i = 1; i < externalRestService.getServiceParameters().size(); i++) {
            uri = uri + "&" + externalRestService.getServiceParameters().get(i).getName() + "=" + externalRestService.getServiceParameters().get(i).getSemantics();
        }
    ...
    private void writeOutputErrorMessage(String message, File logFile, Exception e) {
        if (logFile != null) StringUtil.simpleLog(message, e, this.getClass(), logFile);
        e.printStackTrace();
    }

    private void writeOutputMessage(String message, File logFile) {
        if (logFile != null) StringUtil.simpleLog(message, this.getClass(), logFile);
