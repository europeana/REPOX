...
public class TaskManager implements RunnableStoppable {
        ...
        for (Element scheduledTaskElement : (List<Element>)scheduledTaskElements) {
            ...
            try {
                ...
                if (IngestDataSource.class.isAssignableFrom(taskClass)) {
                    ...
                    taskToRun = new DataSourceExportTask(parameters[0], parameters[1], parameters[2], parameters[3], parameters.length > 4 ? (!parameters[4].isEmpty() ? parameters[4] : null) : null);
                } else {
        ...
        for (Element scheduledTaskElement : scheduledTaskElements) {
            ...
            try {
                ...
                if (IngestDataSource.class.isAssignableFrom(taskClass)) {
                    ...
                    taskToRun = new DataSourceExportTask(parameters[1] + "_" + parameters[0], parameters[1], parameters[2], parameters[3], parameters[4]);
                } else {
                    log.error("Unable to identify Task to handle RunnableStoppable of class: " + taskClass.getName());
                    continue;
                }
    ...
    private void startTask(Task currentTask) {
        log.info("Starting task of class: " + currentTask.getTaskClass().getName() + " with parameters " + StringUtil.getArrayAsString(currentTask.getParameters()));
        ...
        try {
            ...
            log.error("Error running task of class: " + currentTask.getTaskClass().getName() + " with parameters " + StringUtil.getArrayAsString(currentTask.getParameters()), e);
        }
    }
    ...
    private boolean isScheduledTaskExecuting(ScheduledTask scheduledTask, Collection<Task> tasks) {
        for (Task currentTask : tasks) {
            if (scheduledTask.equalActionParameters(currentTask)) { return true; }
        }
        return false;
    }
        ...
        if (runningTasks != null) {
            ...
            while (runningTasksIterator.hasNext()) {
                ...
                if (!task.isRunning()) { //means it has finished
                    if (task.getStatus() != null && task.getStatus().equals(Task.Status.FAILED) && task.getRetries() < task.getMaxRetries()) { // something went wrong, restart if retry is configured
                        ...
                        if (task instanceof DataSourceTask) {
                            ...
                            try {
                                ...
                                log.error("Error saving DataSourceTask with id " + dSTask.getTaskId() + " of DataSource " + dSTask.getDataSourceId(), e);
                            }
                        }
                    }
    ...
    public boolean isTaskExecuting(Task newTask) {
        if (onetimeTasks != null) {
            for (Task task : onetimeTasks) {
                if (task.equalActionParameters(newTask)) { return true; }
            }
        }
    ...
    private void saveTasks() throws IOException {
        ...
        for (ScheduledTask scheduledTask : scheduledTasks) {
            ...
            for (int i = 1; i < scheduledTask.getParameters().length; i++) {
                ...
                if (parameter != null && !parameter.isEmpty()) parametersElement.addElement("parameter").setText(parameter);
            }
