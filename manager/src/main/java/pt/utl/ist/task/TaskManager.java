...
public class TaskManager implements RunnableStoppable {
   ...
   * 
   * @param scheduledTasksFile
   * @param runningTasksFile
   ...
   */
  public TaskManager(File scheduledTasksFile, File runningTasksFile) throws DocumentException,
      IllegalFileFormatException, ParseException, ClassNotFoundException, NoSuchMethodException {
    super();
  ...
  private static List<ScheduledTask> loadNewScheduledTasks(List<Element> scheduledTaskElements)
      throws ParseException, IllegalFileFormatException {
    ...
    for (Element scheduledTaskElement : (List<Element>) scheduledTaskElements) {
      ...
      firstRun.setTime(DateUtil.string2Date(timeElement.getText(),
      ...
      String runnable = scheduledTaskElement.element("runnable").attributeValue("class"); // class
                                                                                          // implementing
                                                                                          // RunnableStoppable
      ...
      try {
        ...
        if (IngestDataSource.class.isAssignableFrom(taskClass)) {
          ...
          taskToRun =
              new DataSourceExportTask(parameters[0], parameters[1], parameters[2], parameters[3],
                  parameters.length > 4 ? (!parameters[4].isEmpty() ? parameters[4] : null) : null);
        } else {
          log.error("Unable to identify Task to handle RunnableStoppable of class: "
              + taskClass.getName());
          continue;
        }

        ScheduledTask scheduledTask =
            new ScheduledTask(id, firstRun, frequency, xmonths, taskToRun);
        loadedScheduledTasks.add(scheduledTask);
      } catch (Exception e) {
        throw new IllegalFileFormatException("Unable to create constructor for class " + runnable
            + " with " + parameters.length + " arguments", e);
      }
  ...
  private static List<ScheduledTask> loadOldScheduledTasks(List<Element> scheduledTaskElements)
      throws ParseException, IllegalFileFormatException {
    List<ScheduledTask> loadedScheduledTasks = new ArrayList<ScheduledTask>();
    for (Element scheduledTaskElement : scheduledTaskElements) {
      ...
      try {
        ...
        if (IngestDataSource.class.isAssignableFrom(taskClass)) {
          taskToRun =
              new DataSourceIngestTask(parameters[1] + "_" + parameters[0], parameters[1],
                  parameters[2]);
        } else if (ExportToFilesystem.class.isAssignableFrom(taskClass)) {
          taskToRun =
              new DataSourceExportTask(parameters[1] + "_" + parameters[0], parameters[1],
        ...
        ScheduledTask scheduledTask =
            new ScheduledTask(parameters[1] + "_" + parameters[0], firstRun, frequency, xmonths,
                taskToRun);
        loadedScheduledTasks.add(scheduledTask);
      } catch (Exception e) {
        throw new IllegalFileFormatException("Unable to create constructor for class " + runnable
  ...
  private void runDataSourcesRecordCount(GregorianCalendar now) {
    ...
    if (countThread == null)
      countThread = new Thread(recordCountManager);
  ...
  /**
   * Returns true if an equivalent scheduled task (same parameters even if it's a different task) is
   * executing and false otherwise. This means that there can't be two scheduled tasks of the same
   * type executing at the same time.
   */
  ...
  private void checkFinishedTasks() throws IOException, ClassNotFoundException, DocumentException,
      ...
      while (runningTasksIterator.hasNext()) {
        ...
        if (!task.isRunning()) { // means it has finished
          if (task.getStatus() != null && task.getStatus().equals(Task.Status.FAILED)
              && task.getRetries() < task.getMaxRetries()) { // something went wrong, restart if
                                                             // retry is configured
            ...
            if (task instanceof DataSourceTask) {
              ...
              try {
                ...
                DataSource dataSource =
                    dataManager.getDataSourceContainer(dSTask.getDataSourceId()).getDataSource();
  ...
  /**
   * @param newTask
   * @return boolean indicating if the newTask is executing
   */
   ...
   * @throws ParseException
   */
  public void addOnetimeTask(Task task) throws ClassNotFoundException, IOException,
      DocumentException, NoSuchMethodException, ParseException {
   ...
   * @throws ParseException
   */
  public void removeOnetimeTask(Task task) throws ClassNotFoundException, IOException,
      DocumentException, NoSuchMethodException, ParseException {
  ...
  private void saveTasks() throws IOException {
    ...
    for (ScheduledTask scheduledTask : scheduledTasks) {
      ...
      timeElement.setText(DateUtil.date2String(scheduledTask.getFirstRun().getTime(),
          TimeUtil.LONG_DATE_FORMAT_NO_SECS));
      ...
      scheduledTaskElement.addElement("runnable").addAttribute("class",
          scheduledTask.getTaskClass().getName());
  ...
  public synchronized boolean deleteTask(String id) throws IOException {
    ...
    if (configurationFile.exists()) {
      ...
      File backupFile =
          new File(configurationFile.getParent(), configurationFile.getName() + ".bkp");
   ...
   * @throws IOException
   */
  public void updateDataSourceTasks(String oldDataSourceId, String newDataSourceId)
      throws IOException {
  ...
  /**
   * @param day
   * @return List of Shceduled Tasks for the day provided
   */
  ...
  /**
   * Checks if a task for a specific time and data source is already scheduled Compares dataSource
   * ID, frequency, date and time - works for both export and ingest tasks
   * 
   * @param dataSourceId
   * @param time
   * @param frequency
   ...
   * @throws IOException
   */
  public boolean taskAlreadyExists(String dataSourceId, String time, Frequency frequency)
      throws IOException {
    for (ScheduledTask scheduledTask : scheduledTasks) {
      if (scheduledTask.getParameters().length > 1
          ...
          && DateUtil.date2String(scheduledTask.getFirstRun().getTime(),
              TimeUtil.LONG_DATE_FORMAT_NO_SECS).equals(time)) {
        return true;
      }
    }
    return false;
