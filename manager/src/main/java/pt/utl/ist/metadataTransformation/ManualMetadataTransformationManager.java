...
public class ManualMetadataTransformationManager {
    ...
    /** ManualMetadataTransformationManager OAI_DC_NAMESPACES */
    ...
    /** ManualMetadataTransformationManager OAI_DC_ROOT */
    public static final String       OAI_DC_ROOT             = "oai_dc:dc";
    /** ManualMetadataTransformationManager TEL_NAMESPACES */
    ...
    /** ManualMetadataTransformationManager TEL_ROOT */
    public static final String       TEL_ROOT                = "record";

    //TODO: confirm that xsl:stylesheet attribute xmlns:rpx = record attribute
    private static final String      templateStart           = "<?xml version='1.0' encoding='UTF-8'?>" + " <xsl:stylesheet version='1.0' xmlns:xsl='http://www.w3.org/1999/XSL/Transform'" + " xmlns:xsi='http://www.w3.org/2001/XMLSchema-instance' " + NAMESPACE_REPLACE + ">" + " <xsl:output method='xml' indent='yes'/>" + " <xsl:template match='/'>";
    ...
    /*
     * <xsl:if test="[tag Xpath]"> <xsl:if test="$currentGroupMatch != 'true'">
     * -> usar hash do xpath do grupo para a variável para evitar problemas
     * <xsl:variable name="currentGroupMatch" select="'true'" /> [initialPrefix]
     * </xsl:if> [currentPrefix]
     * 
     * <xsl:value-of select="."/>
     * 
     * System.out.println("ROOT Element: " +
     * sourceDocument.getRootElement().getName());
     * 
     * </xsl:if>
     * 
     ...
     * @param rootElement
     * @param namespaces
     * @return String of transformation text
     * @throws IOException
    ...
    public String getTransformationText(Document sourceDocument, String rootElement, String namespaces) throws IOException, DocumentException, NoSuchAlgorithmException {
            ...
            for (String currentMapping : tagMappings.get(currentTargetField)) {
                ...
                if (currentMapping.startsWith(TagGroup.GROUP_DELIMITER_START)) {
                    ...
                    for (int i = 0; i < group.getTags().size(); i++) {
                        ...
                        String currentXpathToAdd = (indexOfXpath > 0 ? getNamespaceAware(currentXpath.substring(indexOfXpath), useNamespace).substring(1) : getNamespaceAware(currentXpath, useNamespace));
                    ...
                    for (int i = 0; i < group.getTags().size(); i++) {
                        ...
                        String selectXpath = (indexOfXpath > 0 ? getNamespaceAware(currentTag.getXpath().trim().substring(indexOfXpath), useNamespace).substring(1) : getNamespaceAware(currentTag.getXpath().trim(), useNamespace));
                        mappingToAdd += "<xsl:if test=\"" + selectXpath + "\">" + group.getTagPrefixes().get(i) + "<xsl:text xml:space=\"preserve\"> </xsl:text>" + "<xsl:value-of select=\"" + selectXpath + "\"/>" + "</xsl:if>";
                    ...
                    mappingToAdd = (indexOfXpath > 0 ? "<xsl:for-each select=\"" + getNamespaceAware(group.getCommonXpath(), useNamespace) + "\">" : "") + "<xsl:if test=\"" + allTests + "\">" + "<" + currentTargetField + ">" + mappingToAdd + "</" + getEndTag(currentTargetField) + ">" + "</xsl:if>" + (indexOfXpath > 0 ? "</xsl:for-each>" : "") + "\n\n";
                } else {
                    currentMapping = getNamespaceAware(currentMapping, useNamespace);
                    mappingToAdd = "<xsl:for-each select=\"" + currentMapping.trim() + "\">" + "<" + currentTargetField + ">" + "<xsl:value-of select=\".\"/></" + getEndTag(currentTargetField) + ">" + "</xsl:for-each>\n\n";
                }
        ...
        String stylesheetHeader = (useNamespace ? templateStart.replaceFirst(NAMESPACE_REPLACE, NAMESPACE_URI_ATTRIBUTE + "='" + namespace + "'") : templateStart.replaceFirst(NAMESPACE_REPLACE, ""));
     ...
     * @throws NoSuchAlgorithmException
     */
    public String transform(File sourceFile, String rootElement, String namespaces) throws DocumentException, TransformerException, IOException, NoSuchAlgorithmException {
        ...
        if (transformedDoc.getRootElement() != null && transformedDoc.getRootElement().getNamespaceForPrefix(REPOX_NAMESPACE) != null) {
     ...
     * File("/home/dreis/repoxdata/configuration/xslt/unimarc2tel-notbn.xsl")));
     * 
     * DocumentSource source = new DocumentSource(new SAXReader().read(new
     * File("/home/dreis/repoxdata/kbrdir/kbr_1_806783.marcxchange.xml")));
     * DocumentResult result = new DocumentResult(); transformer.transform(
     * source, result );
     * 
     * // return the transformed document Document transformedDoc =
     * result.getDocument(); OutputFormat format =
     * OutputFormat.createPrettyPrint(); XMLWriter writer = new
     * XMLWriter(System.out, format); writer.write(transformedDoc); }
     */
}
