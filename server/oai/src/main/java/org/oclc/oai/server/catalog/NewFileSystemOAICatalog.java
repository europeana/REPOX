...
public class NewFileSystemOAICatalog extends AbstractCatalog {
    ...
    public NewFileSystemOAICatalog(Properties properties) throws IOException {
        ...
        if (homeDir == null) throw new IllegalArgumentException("NewFileSystemOAICatalog." + "homeDir is missing from the properties file");
        ...
        try {
            ...
            for (String aList : list) {
                ...
                if (child.isDirectory() && !"CVS".equals(child.getName())) {
                    ...
                    fileDateMap.put(path, XPathAPI.eval(datestampNode, "string()").str());
    ...
    public Map listRecords(String from, String until, String set, String metadataPrefix) throws CannotDisseminateFormatException, OAIInternalServerError, NoItemsMatchException {
        ...
        while (count < maxListSize && iterator.hasNext()) {
            ...
            if (fileDate.compareTo(from) >= 0 && fileDate.compareTo(until) <= 0 && extension.equals(metadataPrefix) && (setIdentifiers == null || setIdentifiers.contains(path))) {
                try {
