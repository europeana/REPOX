...
public class DataSourceOaiCatalog extends AbstractCatalog {
    ...
    public Map<String, String> getRepoxSchemaLocations(String identifier) throws IdDoesNotExistException, NoMetadataFormatsException {
        ...
        try {
            if (identifier == null || identifier.isEmpty()) {
                ...
                try {
                    ...
                    if (urn.getRecordId() == null || ConfigSingleton.getRepoxContextUtil().getRepoxManager().getAccessPointsManager().getRecord(urn) == null) { throw new IdDoesNotExistException("Record with identifier: " + identifier + " does not exist."); }
                    ...
                    if (metadataSchemaManager.isMetadataSchemaOaiAvailable(dataSource.getMetadataFormat())) schemaLocationsMap.put(dataSource.getMetadataFormat(), dataSource.getNamespace() + " " + dataSource.getSchema());
        ...
        /*
         * catch (Exception e) {
         * log.error("Error retrieving schema locations for identifier: " +
    ...
    /*
     * private String getSchemaLocation(String metadataPrefix) { String
     * schemaURL = null;
     * 
     * if (metadataPrefix != null) { if(metadataPrefix.equals("MarcXchange")) {
     * schemaURL =
     * "info:lc/xmlns/marcxchange-v1 info:lc/xmlns/marcxchange-v1.xsd"; } else
     * if(metadataPrefix.equals("oai_dc")){ schemaURL =
     * "http://www.openarchives.org/OAI/2.0/ http://www.openarchives.org/OAI/2.0/oai_dc.xsd"
     * ; } else if(metadataPrefix.equals("ese")){ schemaURL =
     ...
     * "http://krait.kb.nl/coop/tel/handbook/telterms.html "; } else { schemaURL
     * = " "; } } return schemaURL; }
     */
    ...
    private Map getListRecords(String resumptionToken, boolean fullRecord) throws CannotDisseminateFormatException, NoItemsMatchException, BadArgumentException {
        ...
        if (resumptionParameters.length < 5 || resumptionParameters.length > 7) { throw new IllegalArgumentException("Invalid resumptionToken"); }

        String resumptionId = resumptionParameters[0];
        if (log.isDebugEnabled()) log.debug("resumptionId: " + resumptionId);
        String set = resumptionParameters[1];
        if (log.isDebugEnabled()) log.debug("set: " + set);
        String metadataPrefix = resumptionParameters[2];
        if (log.isDebugEnabled()) log.debug("metadataPrefix: " + metadataPrefix);
    ...
    private Map getListRecords(String from, String until, String set, String metadataPrefix, int offset, int collectionSize, boolean fullRecord) throws CannotDisseminateFormatException, NoItemsMatchException, BadArgumentException {
            ...
            if (set == null || set.isEmpty()) {
                ...
                for (DataProvider provider : ConfigSingleton.getRepoxContextUtil().getRepoxManager().getDataManager().getDataProviders()) {
                    for (DataSourceContainer dataSourceContainer : provider.getDataSourceContainers().values()) {
                        int setTotalRecord = ConfigSingleton.getRepoxContextUtil().getRepoxManager().getRecordCountManager().getRecordCount(dataSourceContainer.getDataSource().getId()).getCount();

                        if (dataSourceContainer.getDataSource().getMetadataFormat().equals(metadataPrefix) && metadataSchemaManager.isMetadataSchemaOaiAvailable(metadataPrefix)) {
                            dataSetIds.add(new OaiSet(dataSourceContainer.getDataSource().getId(), String.valueOf(setTotalRecord), dataSourceContainer.getDataSource().getMetadataFormat()));
                            totalRecordsSize += setTotalRecord;
                        } else if (dataSourceContainer.getDataSource().hasTransformation(metadataPrefix) && metadataSchemaManager.isMetadataSchemaOaiAvailable(metadataPrefix)) {
                ...
                for (OaiSet oaiSet : dataSetIds) {
                    dataSource = ConfigSingleton.getRepoxContextUtil().getRepoxManager().getDataManager().getDataSourceContainer(oaiSet.getDataSetId()).getDataSource();
                    if (dataSource == null) { throw new BadArgumentException(); }

                    if (!isMetadataPrefixValid(metadataPrefix, dataSource)) { throw new CannotDisseminateFormatException(metadataPrefix); }
                    ...
                    if (numberRecords <= maxListSize && difference > 0 && Integer.valueOf(oaiSet.getTotalRecordNumber()) > 0) {
                        ...
                        Map dataSetMap = getRecordsFromDataSet(from, until, oaiSet.getDataSetId(), metadataPrefix, (offset - accumulated) > 0 ? offset - accumulated : 0, fullRecord, dataSource, set, totalRecordsSize, numberRecords == 0 ? -1 : Math.abs(numberRecords - maxListSize), offset,
                ...
                if (dataSourceContainer == null) { throw new BadArgumentException(); }
                dataSource = ConfigSingleton.getRepoxContextUtil().getRepoxManager().getDataManager().getDataSourceContainer(set).getDataSource();

                if (!isMetadataPrefixValid(metadataPrefix, dataSource) || !metadataSchemaManager.isMetadataSchemaOaiAvailable(metadataPrefix)) { throw new CannotDisseminateFormatException(metadataPrefix); }
                ...
                Map dataSetMap = getRecordsFromDataSet(from, until, set, metadataPrefix, offset, fullRecord, dataSource, set, -1, -1, -1, collectionSize, transformationResultLogger);
                listObjectsMap.put("resumptionMap", dataSetMap.get("resumptionMap"));
                if (fullRecord) {
                    finalItems.addAll((List<String>)dataSetMap.get("records"));
                } else {
    ...
    private Map getRecordsFromDataSet(String from, String until, String set, String metadataPrefix, int offset, boolean fullRecord, DataSource dataSource, String offsetSet, int totalRecordsSize, int maxNumRecords2Return, int originalOffset, int totalSetRecords,
        ...
        for (OaiItem currentItem : oaiListResponse.getOaiItems()) { // load the headers and identifiers ArrayLists.
            try {
                ...
                oaiRecordHeader.append("><" + OAIUtil.getTag("identifier") + ">").append(encodedIdentifier).append("</" + OAIUtil.getTag("identifier") + ">").append("<" + OAIUtil.getTag("datestamp") + ">").append(currentItem.getDatestamp()).append("</" + OAIUtil.getTag("datestamp") + ">").append("<" + OAIUtil.getTag("setSpec") + ">").append(currentItem.getSetSpec()).append("</" + OAIUtil.getTag("setSpec") + "></" + OAIUtil.getTag("header") + ">");
                ...
                if (!fullRecord) {
                    ...
                    if (!currentItem.isDeleted()) {
                        ...
                        xmlRecordString = getTransformedRecord(encodedIdentifier, metadataPrefix, dataSource, xmlRecordString);
                ...
                log.error("Error getting Record in the correct format -" + " Set: " + currentItem.getSetSpec() + " ID: " + currentItem.getIdentifier(), e);
                transformationResultLogger.addRecordErrorEntry(currentItem.getIdentifier(), e.getMessage());
            }
        }
        if (counter == 0) throw new NoItemsMatchException();
        ...
        if (maxNumRecords2Return == -1) {
            if (oaiListResponse.getOaiItems().size() > maxListSize && totalRecordsSize == -1) {
                String resumptionToken = getResumptionToken(offsetSet, metadataPrefix, oaiListResponse.getLastRequestedIdentifier(), offsetSet.isEmpty() ? String.valueOf(totalRecordsSize) : String.valueOf(totalSetRecords), from, until);
            ...
            } else if (oaiListResponse.getOaiItems().size() > maxListSize && totalRecordsSize > (originalOffset + maxListSize)) {
                String resumptionToken = getResumptionToken(offsetSet, metadataPrefix, originalOffset + maxListSize, offsetSet.isEmpty() ? String.valueOf(totalRecordsSize) : String.valueOf(totalSetRecords), from, until);
    ...
    private String getOaiProvenance(OaiDataSource dataSource, OaiItem oaiItem) {
        ...
        StringBuffer originDescription = new StringBuffer("<oaiProvenance:originDescription harvestDate=\"").append(oaiItem.getDatestamp()).append("\" altered=\"true\">").append("<oaiProvenance:baseURL>").append(dataSource.getOaiSourceURL()).append("</oaiProvenance:baseURL>").append("<oaiProvenance:identifier>").append(oaiItem.getIdentifier()).append("</oaiProvenance:identifier>").append("<oaiProvenance:datestamp>").append(oaiItem.getDatestamp()).append("</oaiProvenance:datestamp>").append("<oaiProvenance:metadataNamespace>").append(metadataNamespace).append("</oaiProvenance:metadataNamespace>").append("</oaiProvenance:originDescription>");

        oaiProvenance.append("<" + OAIUtil.getTag("about") + "><oaiProvenance:provenance xmlns:oaiProvenance=\"http://www.openarchives.org/OAI/2.0/provenance\"").append(" xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\"").append("  xsi:schemaLocation=\"http://www.openarchives.org/OAI/2.0/provenance ").append("http://www.openarchives.org/OAI/2.0/provenance.xsd\">").append(originDescription.toString()).append("</oaiProvenance:provenance></" + OAIUtil.getTag("about") + ">");
    ...
    public String getRecord(String identifier, String metadataPrefix) throws CannotDisseminateFormatException, IdDoesNotExistException {
        ...
        try {
            ...
            try {
                if (urn == null || urn.getRecordId() == null || ConfigSingleton.getRepoxContextUtil().getRepoxManager().getAccessPointsManager().getRecord(urn) == null) { throw new IdDoesNotExistException("Record with identifier: " + identifier + " does not exist."); }
        ...
        try {
            ...
            String encodedIdentifier = OAIUtil.xmlEncode(identifier);
            oaiRecordHeader = oaiRecordHeader + "><" + OAIUtil.getTag("identifier") + ">" + encodedIdentifier + "</" + OAIUtil.getTag("identifier") + ">" + "<" + OAIUtil.getTag("datestamp") + ">" + oaiItem.getDatestamp() + "</" + OAIUtil.getTag("datestamp") + ">" + "<" + OAIUtil.getTag("setSpec") + ">" + urn.getDataSourceId() + "</" + OAIUtil.getTag("setSpec") + "></" + OAIUtil.getTag("header") + ">";
    ...
    private boolean isMetadataPrefixValid(String metadataPrefix, DataSource dataSource) {
        ...
        if (metadataPrefix.equals("MarcXchange") && dataSource.getMetadataFormat().equals("ISO2709")) {
            isValid = true;
        } else if (dataSource == null || (!dataSource.getMetadataFormat().equals(metadataPrefix) && !dataSource.hasTransformation(metadataPrefix))) {
            isValid = false;
        }
    ...
    public Map listSets() {
        ...
        try {
            ...
            for (DataSourceContainer dataSourceContainer : dataSourceContainers.values()) {
                sets.add("<" + OAIUtil.getTag("set") + "><" + OAIUtil.getTag("setSpec") + ">" + dataSourceContainer.getDataSource().getId() + "</" + OAIUtil.getTag("setSpec") + "><" + OAIUtil.getTag("setName") + ">" + dataSourceContainer.getDataSource().getDescription() + "</" + OAIUtil.getTag("setName") + "></" + OAIUtil.getTag("set") + ">");
            }
        } catch (Exception e) {
            log.error(e.getMessage(), e);
            return null;
    ...
    public Map listSets(String resumptionToken) throws BadResumptionTokenException {
        ...
        if (dbSets == null) { throw new BadResumptionTokenException(); }
        int count;
        ...
        try {
            ...
            for (DataSource dataSource : dataSources) {
                ...
                while (offset < dataSetSize) {
                    Map listIdentifiersMap = getRecordsFromDataSet(null, null, dataSource.getId(), metadataPrefix, offset, false, dataSource, dataSource.getId(), -1, -1, offset, dataSetSize, new TransformationResultLogger());
