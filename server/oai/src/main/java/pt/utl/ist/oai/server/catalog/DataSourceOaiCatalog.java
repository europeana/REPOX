/**
 * Copyright (c) 2000-2002 OCLC Online Computer Library Center, Inc. and other contributors. All
 * rights reserved. The contents of this file, as updated from time to time by the OCLC Office of
 * Research, are subject to OCLC Research Public License Version 2.0 (the "License"); you may not
 * use this file except in compliance with the License. You may obtain a current copy of the License
 * at http://purl.oclc.org/oclc/research/ORPL/. Software distributed under the License is
 ...
 * For more information on OCLC Research, please see http://www.oclc.org/oclc/research/.
 *
 * The Original Code is DummyOAICatalog.java. The Initial Developer of the Original Code is Jeff
 * Young. Portions created by Diogo Mena Reis
 */
...
public class DataSourceOaiCatalog extends AbstractCatalog {
  ...
  public DataSourceOaiCatalog(Properties properties) {
    ...
    if (maxListSize == null) {
      throw new IllegalArgumentException(
          "DataSourceOAICatalog.maxListSize is missing from the properties file");
    } else {
   ...
   * @exception NoMetadataFormatsException the specified identifier was found but the item is
   *            flagged as deleted and thus no schemaLocations (i.e. metadataFormats) can be
   *            produced.
   * @throws ObjectNotFoundException
   */
  public Map<String, String> getRepoxSchemaLocations(String identifier)
      throws IdDoesNotExistException, NoMetadataFormatsException, ObjectNotFoundException {
    ...
    try {
      if (identifier == null || identifier.isEmpty()) {
        ...
        for (DataSourceContainer dataSourceContainer : dataSourceContainers.values()) {
          ...
          for (MetadataTransformation metadataTransformation : dataSourceContainer.getDataSource()
              .getMetadataTransformations().values()) {
            if (metadataSchemaManager.isMetadataSchemaOaiAvailable(metadataTransformation
                .getDestinationSchemaId()))
              schemaLocationsMap.put(
                  metadataTransformation.getDestinationSchemaId(),
                  metadataTransformation.getDestNamespace() + " "
                      + metadataTransformation.getDestSchema());
          }

          if (metadataSchemaManager.isMetadataSchemaOaiAvailable(dataSourceContainer
              .getDataSource().getMetadataFormat()))
            schemaLocationsMap.put(dataSourceContainer.getDataSource().getMetadataFormat(),
                dataSourceContainer.getDataSource().getNamespace() + " "
                    + dataSourceContainer.getDataSource().getSchema());
        }
      } else {
        try {
          Urn urn = new Urn(identifier);
          if (urn.getRecordId() == null
              || ConfigSingleton.getRepoxContextUtil().getRepoxManager().getAccessPointsManager()
                  .getRecord(urn) == null) {
            throw new IdDoesNotExistException("Record with identifier: " + identifier
                + " does not exist.");
          }
          DataSource dataSource =
              ConfigSingleton.getRepoxContextUtil().getRepoxManager().getDataManager()
                  .getDataSourceContainer(urn.getDataSourceId()).getDataSource();
          if (metadataSchemaManager.isMetadataSchemaOaiAvailable(dataSource.getMetadataFormat()))
            schemaLocationsMap.put(dataSource.getMetadataFormat(), dataSource.getNamespace() + " "
    ...
    /*
     * catch (Exception e) { log.error("Error retrieving schema locations for identifier: " +
  ...
  /*
   * private String getSchemaLocation(String metadataPrefix) { String schemaURL = null;
   * 
   * if (metadataPrefix != null) { if(metadataPrefix.equals("MarcXchange")) { schemaURL =
   * "info:lc/xmlns/marcxchange-v1 info:lc/xmlns/marcxchange-v1.xsd"; } else
   * if(metadataPrefix.equals("oai_dc")){ schemaURL =
   * "http://www.openarchives.org/OAI/2.0/ http://www.openarchives.org/OAI/2.0/oai_dc.xsd" ; } else
   * if(metadataPrefix.equals("ese")){ schemaURL =
   * "http://www.europeana.eu/schemas/ese/ http://www.europeana.eu/schemdaas/ese/ESE-V3.3.xsd" ; }
   * else if(metadataPrefix.equals("tel")){ schemaURL =
   * "http://krait.kb.nl/coop/tel/handbook/telterms.html "; } else { schemaURL = " "; } } return
   * schemaURL; }
   */
   ...
   *         containing Strings) as well as an optional "resumptionMap" Map. It may seem strange for
   *         the map to include both "headers" and "identifiers" since the identifiers can be
   *         obtained from the headers. This may be true, but AbstractCatalog.listRecords() can
   *         operate quicker if it doesn't need to parse identifiers from the XML headers itself.
   *         Better still, do like I do below and override AbstractCatalog.listRecords().
   *         AbstractCatalog.listRecords() is relatively inefficient because given the list of
   *         identifiers, it must call getRecord() individually for each as it constructs its
   *         response. It's much more efficient to construct the entire response in one fell swoop
   *         by overriding listRecords() as I've done here.
  ...
  public Map listIdentifiers(String from, String until, String set, String metadataPrefix)
      throws BadArgumentException, NoItemsMatchException, CannotDisseminateFormatException, NumberFormatException, TransformerException {
  ...
  public Map listIdentifiers(String resumptionToken) throws BadResumptionTokenException,
      NoItemsMatchException, TransformerException {
    purge(); // clean out old resumptionTokens
   ...
   * @exception CannotDisseminateFormatException the metadataPrefix isn't supported by the item.
   * @throws BadArgumentException
   * @throws TransformerException 
   * @throws NumberFormatException 
   */
  ...
  public Map listRecords(String from, String until, String set, String metadataPrefix)
      throws CannotDisseminateFormatException, BadArgumentException, NoItemsMatchException, NumberFormatException, TransformerException {
    purge(); // clean out old resumptionTokens
  ...
  private Map getListRecords(String resumptionToken, boolean fullRecord)
      throws CannotDisseminateFormatException, NoItemsMatchException, BadArgumentException, NumberFormatException, TransformerException {
  ...
  private Map getListRecords(String from, String until, String set, String metadataPrefix,
      int offset, int collectionSize, boolean fullRecord) throws CannotDisseminateFormatException,
      NoItemsMatchException, BadArgumentException, NumberFormatException, TransformerException {
    ...
    try {
      ...
      if (set == null || set.isEmpty()) {
        ...
        for (DataProvider provider : ConfigSingleton.getRepoxContextUtil().getRepoxManager()
            .getDataManager().getDataProviders()) {
          for (DataSourceContainer dataSourceContainer : provider.getDataSourceContainers()
              .values()) {
            int setTotalRecord =
            ...
            if (dataSourceContainer.getDataSource().getMetadataFormat().equals(metadataPrefix)
              ...
              dataSetIds
                  .add(new OaiSet(dataSourceContainer.getDataSource().getId(), String
                      .valueOf(setTotalRecord), dataSourceContainer.getDataSource()
                      .getMetadataFormat()));
        ...
        for (OaiSet oaiSet : dataSetIds) {
          dataSource =
              ConfigSingleton.getRepoxContextUtil().getRepoxManager().getDataManager()
                  .getDataSourceContainer(oaiSet.getDataSetId()).getDataSource();
          if (dataSource == null) {
          ...
          if (numberRecords <= maxListSize && difference > 0
              && Integer.valueOf(oaiSet.getTotalRecordNumber()) > 0) {
            ...
            if (currentDataSource.hasTransformation(metadataPrefix)) {
              ...
              for (MetadataTransformation metadataTransformation : currentDataSource
                  .getMetadataTransformations().values()) {
            ...
            Map dataSetMap =
                getRecordsFromDataSet(from, until, oaiSet.getDataSetId(), metadataPrefix,
                    (offset - accumulated) > 0 ? offset - accumulated : 0, fullRecord, dataSource,
                    set, totalRecordsSize,
                    numberRecords == 0 ? -1 : Math.abs(numberRecords - maxListSize), offset,
                    /* collectionSize */Integer.valueOf(oaiSet.getTotalRecordNumber()),
                    transformationResultLogger);
        ...
        DataSourceContainer dataSourceContainer =
            ConfigSingleton.getRepoxContextUtil().getRepoxManager().getDataManager()
                .getDataSourceContainer(set);
        if (dataSourceContainer == null) {
          throw new BadArgumentException();
        }
        dataSource =
            ConfigSingleton.getRepoxContextUtil().getRepoxManager().getDataManager()
                .getDataSourceContainer(set).getDataSource();
        ...
        Map dataSetMap =
            getRecordsFromDataSet(from, until, set, metadataPrefix, offset, fullRecord, dataSource,
  ...
  private Map getRecordsFromDataSet(String from, String until, String set, String metadataPrefix,
      int offset, boolean fullRecord, DataSource dataSource, String offsetSet,
      int totalRecordsSize, int maxNumRecords2Return, int originalOffset, int totalSetRecords,
      TransformationResultLogger transformationResultLogger) throws NoItemsMatchException, TransformerException {
    ...
    try {
      AccessPointsManager accessPointsManager =
          ConfigSingleton.getRepoxContextUtil().getRepoxManager().getAccessPointsManager();

      if ((from != null || until != null) && totalSetRecords == -1) {
        // used to fill the completeListSize field
        totalSetRecords =
            accessPointsManager.getRecordCountLastrowPair(dataSource, 0, from, until)[0];
        // totalSetRecords = accessPointsManager.getOaiRecordsFromDataSource(dataSource, from,
        // until, offset, -1, !fullRecord).getOaiItems().size();
      } else if (totalSetRecords == -1) {
        totalSetRecords =
            ConfigSingleton.getRepoxContextUtil().getRepoxManager().getRecordCountManager()
                .getRecordCount(dataSource.getId()).getCount();
      }

      log.debug("Total number of Records from " + dataSource.getId() + " :" + totalSetRecords);

      // NOTE: we get 1 more record than maxlist for the resumptionToken and to avoid a last request
      // with no results
      oaiListResponse =
          accessPointsManager.getOaiRecordsFromDataSource(dataSource, from, until, offset,
              maxNumRecords2Return != -1 ? maxNumRecords2Return + 1 : maxListSize + 1, !fullRecord);
      log.debug("Total number of Records from specific query"
          + oaiListResponse.getOaiItems().size());
    } catch (Exception e) {
    ...
    for (OaiItem currentItem : oaiListResponse.getOaiItems()) { // load the headers and identifiers
                                                                // ArrayLists.
      try {
        if (counter >= maxListSize
            || (maxNumRecords2Return != -1 && counter >= maxNumRecords2Return)) {
          break;
        }
        counter++;
        String identifier =
            new Urn(currentItem.getSetSpec(), currentItem.getIdentifier()).toString();
        ...
        oaiRecordHeader.append("><" + OAIUtil.getTag("identifier") + ">").append(encodedIdentifier)
            .append("</" + OAIUtil.getTag("identifier") + ">")
            .append("<" + OAIUtil.getTag("datestamp") + ">").append(currentItem.getDatestamp())
            .append("</" + OAIUtil.getTag("datestamp") + ">")
            .append("<" + OAIUtil.getTag("setSpec") + ">").append(currentItem.getSetSpec())
        ...
        if (!fullRecord) {
          ...
          if (!currentItem.isDeleted()) {
            String xmlRecordString =
                (currentItem.getMetadata() != null ? new String(currentItem.getMetadata(), "UTF-8")
                    : "");
            xmlRecordString =
                getTransformedRecord(encodedIdentifier, metadataPrefix, dataSource, xmlRecordString);
            if (xmlRecordString.startsWith("<?xml version=\"1.0\" encoding=\"UTF-8\"?>")) {
              xmlRecordString =
                  xmlRecordString.substring("<?xml version=\"1.0\" encoding=\"UTF-8\"?>".length());
            }
            oaiMetadata.append("<" + OAIUtil.getTag("metadata") + ">").append(xmlRecordString)
                .append("</" + OAIUtil.getTag("metadata") + ">");
          }
          ...
          StringBuffer oaiRecord =
              new StringBuffer("<" + OAIUtil.getTag("record") + ">")
                  .append(oaiRecordHeader.toString()).append(oaiMetadata.toString())
                  .append(oaiProvenance).append("</" + OAIUtil.getTag("record") + ">");
        ...
        transformationResultLogger.addRecordErrorEntry(currentItem.getIdentifier(),
        ...
        log.error(
            "Error getting Record in the correct format -" + " Set: " + currentItem.getSetSpec()
                + " ID: " + currentItem.getIdentifier(), e);
        transformationResultLogger.addRecordErrorEntry(currentItem.getIdentifier(), e.getMessage());
      }
    }
    ...
    // we got one more record than required in the request, if it exists, a resumptionToken is
    // required
      ...
      if (oaiListResponse.getOaiItems().size() > maxListSize && totalRecordsSize == -1) {
        String resumptionToken =
            getResumptionToken(
                offsetSet,
                metadataPrefix,
                oaiListResponse.getLastRequestedIdentifier(),
                offsetSet.isEmpty() ? String.valueOf(totalRecordsSize) : String
                    .valueOf(totalSetRecords), from, until);
        // String resumptionToken = getResumptionToken(offsetSet, metadataPrefix,
        // oaiListResponse.getLastRequestedIdentifier(), String.valueOf(totalSetRecords), from,
        // until);
        dataSetMap.put("resumptionMap",
            getResumptionMap(resumptionToken, /*
                                               * offsetSet.isEmpty()?totalRecordsSize:totalSetRecords
                                               */totalSetRecords, offset));
        ...
        String resumptionToken =
            getResumptionToken(
                offsetSet,
                metadataPrefix,
                originalOffset + maxListSize,
                offsetSet.isEmpty() ? String.valueOf(totalRecordsSize) : String
                    .valueOf(totalSetRecords), from, until);
        // String resumptionToken = getResumptionToken(offsetSet, metadataPrefix, originalOffset +
        // maxListSize, String.valueOf(totalSetRecords), from, until);
        dataSetMap.put(
            "resumptionMap",
            getResumptionMap(resumptionToken, offsetSet.isEmpty() ? totalRecordsSize
                : totalSetRecords, originalOffset));
      }
    } else {
  ...
  private String getOaiProvenance(OaiDataSource dataSource, OaiItem oaiItem) {
    ...
    // try {
    // Namespace namespace =
    // DocumentHelper.parseText(xmlRecordString).getRootElement().getDestNamespace();
    ...
    StringBuffer originDescription =
        new StringBuffer("<oaiProvenance:originDescription harvestDate=\"")
            .append(oaiItem.getDatestamp()).append("\" altered=\"true\">")
            .append("<oaiProvenance:baseURL>").append(dataSource.getOaiSourceURL())
            .append("</oaiProvenance:baseURL>").append("<oaiProvenance:identifier>")
            .append(oaiItem.getIdentifier()).append("</oaiProvenance:identifier>")
            .append("<oaiProvenance:datestamp>").append(oaiItem.getDatestamp())
            .append("</oaiProvenance:datestamp>").append("<oaiProvenance:metadataNamespace>")
            .append(metadataNamespace).append("</oaiProvenance:metadataNamespace>")
            .append("</oaiProvenance:originDescription>");

    oaiProvenance
        .append(
            "<"
                + OAIUtil.getTag("about")
                + "><oaiProvenance:provenance xmlns:oaiProvenance=\"http://www.openarchives.org/OAI/2.0/provenance\"")
   ...
   *****************************************************************/
  private String getResumptionToken(String set, String metadataPrefix, int offset,
      String collectionSize, String from, String until) {
   ...
   * @exception CannotDisseminateFormatException the metadataPrefix is not supported by the item.
  ...
  public String getRecord(String identifier, String metadataPrefix)
    ...
    try {
      ...
      try {
        for (String dataSourceId : ConfigSingleton.getRepoxContextUtil().getRepoxManager()
            .getDataManager().loadDataSourceContainers().keySet()) {
          DataSource dataSourceTemp =
              ConfigSingleton.getRepoxContextUtil().getRepoxManager().getDataManager()
                  .loadDataSourceContainers().get(dataSourceId).getDataSource();
          if (isMetadataPrefixValid(metadataPrefix, dataSourceTemp)) {
            dataSources.add(dataSourceTemp);
          }
        }
        HashMap<String, DataSource> element =
            getIdBySuffix(dataSources, metadataPrefix, identifier);
        if (element == null)
          return null;
    ...
    try {
      ...
      RecordRepox recordRepox =
          dataSource.getRecordIdPolicy().createRecordRepox(rootElement,
              urn.getRecordId().toString(), false, oaiItem.isDeleted());
      ...
      String encodedIdentifier = OAIUtil.xmlEncode(identifier);
      oaiRecordHeader =
          oaiRecordHeader + "><" + OAIUtil.getTag("identifier") + ">" + encodedIdentifier + "</"
              + OAIUtil.getTag("identifier") + ">" + "<" + OAIUtil.getTag("datestamp") + ">"
              + oaiItem.getDatestamp() + "</" + OAIUtil.getTag("datestamp") + ">" + "<"
              + OAIUtil.getTag("setSpec") + ">" + urn.getDataSourceId() + "</"
              + OAIUtil.getTag("setSpec") + "></" + OAIUtil.getTag("header") + ">";

      String xmlRecordString = "";

      if (!recordRepox.isDeleted()) {
        xmlRecordString =
            getTransformedRecord(encodedIdentifier, metadataPrefix, dataSource, recordRepox
                .getDom().asXML());
        if (xmlRecordString.startsWith("<?xml version=\"1.0\" encoding=\"UTF-8\"?>")) {
          xmlRecordString =
              xmlRecordString.substring(new String("<?xml version=\"1.0\" encoding=\"UTF-8\"?>")
                  .length());
        }
      }

      String oaiMetadata =
          (recordRepox.isDeleted() ? "" : "<" + OAIUtil.getTag("metadata") + ">" + xmlRecordString
              + "</" + OAIUtil.getTag("metadata") + ">");
      ...
      // String oaiRecord = "<" + OAIUtil.getTag("record") +
      // " xmlns:oai=\"http://www.openarchives.org/OAI/2.0/\">" + oaiRecordHeader + oaiMetadata +
      // oaiProvenance + "</" + OAIUtil.getTag("record") + ">";
      String oaiRecord =
          "<"
              + OAIUtil.getTag("record")
              + " xmlns"
              + (ConfigSingleton.getRepoxContextUtil().getRepoxManager().getConfiguration()
                  .isUseOAINamespace() ? ":oai" : "")
              + "=\"http://www.openarchives.org/OAI/2.0/\">" + oaiRecordHeader + oaiMetadata
              + oaiProvenance + "</" + OAIUtil.getTag("record") + ">";
   ...
   */
  public static String getTransformedRecord(String encodedIdentifier, String metadataPrefix,
      DataSource dataSource, String xmlRecordString) throws DocumentException,
    ...
    try {
      ...
      } else if (!dataSource.getMetadataFormat().equals(metadataPrefix)
          && !xmlRecordString.isEmpty()) {

        for (MetadataTransformation metadataTransformation : dataSource
            .getMetadataTransformations().values()) {
          if (metadataTransformation.getDestinationSchemaId().equals(metadataPrefix)) {
            DataProvider dataProviderParent =
                ConfigSingleton.getRepoxContextUtil().getRepoxManager().getDataManager()
                    .getDataProviderParent(dataSource.getId());
            xmlRecordString =
                metadataTransformation.transform(encodedIdentifier, xmlRecordString,
                    dataProviderParent.getName());
            return xmlRecordString;
          }
        }
      }
  ...
  private boolean isMetadataPrefixValid(String metadataPrefix, DataSource dataSource) {
    ...
    if (metadataPrefix.equals("MarcXchange") && dataSource.getMetadataFormat().equals("ISO2709")) {
      isValid = true;
        ...
        || (!dataSource.getMetadataFormat().equals(metadataPrefix) && !dataSource
            .hasTransformation(metadataPrefix))) {
      isValid = false;
    }
  ...
  public Map listSets() {
    ...
    try {
      ...
      for (DataSourceContainer dataSourceContainer : dataSourceContainers.values()) {
        sets.add("<" + OAIUtil.getTag("set") + "><" + OAIUtil.getTag("setSpec") + ">"
            + dataSourceContainer.getDataSource().getId() + "</" + OAIUtil.getTag("setSpec") + "><"
            + OAIUtil.getTag("setName") + ">"
            + dataSourceContainer.getDataSource().getDescription() + "</"
            + OAIUtil.getTag("setName") + "></" + OAIUtil.getTag("set") + ">");
      }
    } catch (Exception e) {
      log.error(e.getMessage(), e);
      return null;
  ...
  public Map listSets(String resumptionToken) throws BadResumptionTokenException {
     ...
     **********************************************************************/
    /**********************************************************************
     * parse your resumptionToken and look it up in the resumptionResults, if necessary
     **********************************************************************/
    ...
    if (count + oldCount < dbSets.length) {
      ...
      /*****************************************************************
       * Store an object appropriate for your database API in the resumptionResults Map in place of
       * nativeItems. This object should probably encapsulate the information necessary to perform
       * the next resumption of ListIdentifiers. It might even be possible to encode everything you
       * need in the resumptionToken, in which case you won't need the resumptionResults Map. Here,
       * I've done a silly combination of the two. Stateless resumptionTokens have some advantages.
       *****************************************************************/
      ...
      /*****************************************************************
       * Use the following line if you wish to include the optional resumptionToken attributes in
       * the response. Otherwise, use the line after it that I've commented out.
       *****************************************************************/
      listSetsMap.put("resumptionMap",
          getResumptionMap(resumptionTokenSb.toString(), dbSets.length, oldCount));
      // listSetsMap.put("resumptionMap",
  ...
  private HashMap<String, DataSource> getIdBySuffix(List<DataSource> dataSources,
      String metadataPrefix, String id) {
    ...
    try {
      ...
      for (DataSource dataSource : dataSources) {
        ...
        while (offset < dataSetSize) {
          Map listIdentifiersMap =
              getRecordsFromDataSet(null, null, dataSource.getId(), metadataPrefix, offset, false,
                  dataSource, dataSource.getId(), -1, -1, offset, dataSetSize,
