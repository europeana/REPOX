...
//            } 
            ...
            if ((restRequest.getUriHierarchy() != null) && !restRequest.getUriHierarchy().isEmpty() && restRequest.getUriHierarchy().get(0).equals("createMapping")) {
                ...
                if (id != null && !id.isEmpty() && xslFilename != null && !xslFilename.isEmpty() && description != null && !description.isEmpty() && srcSchemaId != null && !srcSchemaId.isEmpty() && srcSchemaVersion != null && !srcSchemaVersion
                        .isEmpty() && destSchemaId != null && !destSchemaId.isEmpty() && destSchemaVersion != null && !destSchemaVersion.isEmpty() && isXslVersion2 != null && !isXslVersion2.isEmpty() && xsdFile != null && xsdFile
                        .available() != 0) {
                ...
                if (id != null && !id.isEmpty()) {
                    webServices.updateMapping(out, id, description, srcSchemaId, srcSchemaVersion, destSchemaId, destSchemaVersion, isXslVersion2, xslFilename, xsdFile,
                            oldMappingId == null ? id : oldMappingId);
                } else {
                    webServices
                            .createErrorMessage(
                                    out,
                                    MessageType.INVALID_REQUEST,
                                    "Error creating mapping:" + "invalid arguments. Syntax: /rest/updateMapping?id=ID&oldId=OLD_ID" + "&description=DESCRIPTION&srcSchemaId=SOURCE_SCHEMA_ID&srcSchemaVersion=SOURCE_SCHEMA_VERSION" + "&destSchemaId=DESTINATION_SCHEMA_ID&destSchemaVersion=DESTINATION_SCHEMA_VERSION " + "&isXslVersion2=TRUE/FALSE&xslFilename=XSL_FILE_NAME" + "[Mandatory fields: all the above]" + " NOTE: XSL FILE MUST BE UPLOADED THROUGH HTTP POST");
                }
                ...
                if (restRequest.getUriHierarchy().get(1).equals("createZ3950IdList")) {
                    ...
                    if (dataProviderId != null && !dataProviderId.isEmpty() && id != null && !id.isEmpty() && description != null && !description.isEmpty() && schema != null && !schema.isEmpty() && namespace != null && !namespace
                            .isEmpty() && address != null && !address.isEmpty() && port != null && !port.isEmpty() && database != null && !database.isEmpty() && recordSyntax != null && !recordSyntax
                            .isEmpty() && charset != null && !charset.isEmpty() && xsdFile != null && xsdFile.available() != 0 && recordIdPolicy != null && !recordIdPolicy.isEmpty()) {
                        ((DefaultWebServices)webServices).createDataSourceZ3950IdList(out, dataProviderId, id, description, nameCode, name, exportPath, schema, namespace, address, port, database,
                                user, password, recordSyntax, charset, xsdFile, recordIdPolicy, idXpath, namespacePrefix, namespaceUri);
                    } else {
                        webServices
                                .createErrorMessage(
                                        out,
                    ...
                    if (id != null && !id.isEmpty()) {
                        ((DefaultWebServices)webServices).updateDataSourceZ3950IdList(out, id, description, nameCode, name, exportPath, schema, namespace, address, port, database, user, password,
                                recordSyntax, charset, xsdFile, recordIdPolicy, idXpath, namespacePrefix, namespaceUri);
                    } else {
                        webServices
                                .createErrorMessage(
                                        out,
                                        MessageType.INVALID_REQUEST,
                                        "Error updating the" + "Z39.50 Data Source with ID list: invalid arguments." + "Syntax: /rest/dataSources/updateZ3950IdList?id=DATA_SOURCE_ID&description=DESCRIPTION" + "&nameCode=NAME_CODE&name=NAME&exportPath=EXPORT_PATH" + "&schema=SCHEMA&namespace=NAMESPACE" + "&address=ADDRESS&port=PORT&database=DATABASE&user=USER&password=PASSWORD" + "&recordSyntax=RECORDS_SYNTAX&charset=CHARSET&filePath=FILE_PATH" + "&recordIdPolicy=RECORD_ID_POLICY&idXpath=ID_XPATH" + "&namespacePrefix=NAMESPACE_PREFIX&namespaceUri=NAMESPACE_URI [Mandatory field: " + "id (if recordIdPolicy=IdExtracted the fields idXpath, namespacePrefix and namespaceUri " + "are mandatory)]");
                    }
                }
            } else {
    ...
    private boolean validateRecordUrn(RestRequest restRequest, ServletOutputStream out, String recordUrnString) throws IOException, DocumentException, SQLException, ObjectNotFoundException {
        ...
        return !(!validateDataSource(restRequest, out, recordUrn.getDataSourceId()) || !validateRecordId(restRequest, out, recordUrnString) || ConfigSingleton.getRepoxContextUtil().getRepoxManager()
                .getAccessPointsManager().getRecord(recordUrn) == null);
