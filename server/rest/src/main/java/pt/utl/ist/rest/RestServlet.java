...
public class RestServlet extends HttpServlet {
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try {
            if (projectType == ProjectType.LIGHT) {
                LightWebServices lightWebServices = new LightWebServices();
                responseRest = new LightResponseRest();
                responseRest.response(request, response, lightWebServices);
            } else if (projectType == ProjectType.DEFAULT) {
                DefaultWebServices defaultWebServices = new DefaultWebServices();
                responseRest = new DefaultResponseRest();
                responseRest.response(request, response, defaultWebServices);
            }
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try {
            ...
            if ((restRequest.getUriHierarchy() != null) && !restRequest.getUriHierarchy().isEmpty() && restRequest.getUriHierarchy().get(0).equals("createMapping")) {
                ...
                if (id != null && !id.isEmpty() && xslFilename != null && !xslFilename.isEmpty() && description != null && !description.isEmpty() && srcSchemaId != null && !srcSchemaId.isEmpty() && srcSchemaVersion != null && !srcSchemaVersion.isEmpty() && destSchemaId != null && !destSchemaId.isEmpty() && destSchemaVersion != null && !destSchemaVersion.isEmpty() && isXslVersion2 != null && !isXslVersion2.isEmpty() && xsdFile != null && xsdFile.available() != 0) {
                    ...
                    webServices.createErrorMessage(out, MessageType.INVALID_REQUEST, "Error creating mapping:" + "invalid arguments. Syntax: /rest/createMapping?id=ID" + "&description=DESCRIPTION&srcSchemaId=SOURCE_SCHEMA_ID&srcSchemaVersion=SOURCE_SCHEMA_VERSION" + "&destSchemaId=DESTINATION_SCHEMA_ID&destSchemaVersion=DESTINATION_SCHEMA_VERSION " + "&isXslVersion2=TRUE/FALSE&xslFilename=XSL_FILE_NAME" + "[Mandatory fields: all the above]" + " NOTE: XSL FILE MUST BE UPLOADED THROUGH HTTP POST");
                }
            } else if ((restRequest.getUriHierarchy() != null) && !restRequest.getUriHierarchy().isEmpty() && restRequest.getUriHierarchy().get(0).equals("updateMapping")) {
                String id = restRequest.getRequestParameters().get("id");
                ...
                if (id != null && !id.isEmpty()) {
                    ...
                    webServices.createErrorMessage(out, MessageType.INVALID_REQUEST, "Error creating mapping:" + "invalid arguments. Syntax: /rest/updateMapping?id=ID&oldId=OLD_ID" + "&description=DESCRIPTION&srcSchemaId=SOURCE_SCHEMA_ID&srcSchemaVersion=SOURCE_SCHEMA_VERSION" + "&destSchemaId=DESTINATION_SCHEMA_ID&destSchemaVersion=DESTINATION_SCHEMA_VERSION " + "&isXslVersion2=TRUE/FALSE&xslFilename=XSL_FILE_NAME" + "[Mandatory fields: all the above]" + " NOTE: XSL FILE MUST BE UPLOADED THROUGH HTTP POST");
                }
            } else if ((restRequest.getUriHierarchy() != null) && restRequest.getUriHierarchy().size() == 1 && restRequest.getUriHierarchy().get(0).equals(RECORDS_URL_NAME)) {
                ...
                if (operationParameter != null && operationParameter.equals("save") && recordParameter != null && !recordParameter.trim().isEmpty()) { // Save Record
                ...
                } else if (operationParameter != null && operationParameter.equals("delete") && validateRecordId(restRequest, out, recordIdParameter)) { // Mark record as deleted
                    webServices.deleteRecord(out, recordIdParameter);
                } else if (operationParameter != null && operationParameter.equals("erase") && validateRecordId(restRequest, out, recordIdParameter)) { // Permanently remove record
                ...
                if (restRequest.getUriHierarchy().get(1).equals("createZ3950IdList")) {
                    ...
                    if (dataProviderId != null && !dataProviderId.isEmpty() && id != null && !id.isEmpty() && description != null && !description.isEmpty() && schema != null && !schema.isEmpty() && namespace != null && !namespace.isEmpty() && address != null && !address.isEmpty() && port != null && !port.isEmpty() && database != null && !database.isEmpty() && recordSyntax != null && !recordSyntax.isEmpty() && charset != null && !charset.isEmpty() && xsdFile != null && xsdFile.available() != 0 && recordIdPolicy != null && !recordIdPolicy.isEmpty()) {
                        ((DefaultWebServices)webServices).createDataSourceZ3950IdList(out, dataProviderId, id, description, nameCode, name, exportPath, schema, namespace, address, port, database, user, password, recordSyntax, charset, xsdFile, recordIdPolicy, idXpath, namespacePrefix, namespaceUri);
                    } else {
                        webServices.createErrorMessage(out, MessageType.INVALID_REQUEST, "Error creating the Data" + "Source: invalid arguments." + "Syntax: /rest/dataSources/createZ3950IdList?dataProviderId=DATA_PROVIDER_ID" + "&id=DATA_SOURCE_ID&description=DESCRIPTION" + "&nameCode=NAME_CODE&name=NAME&exportPath=EXPORT_PATH" + "&schema=SCHEMA&namespace=NAMESPACE" + "&address=ADDRESS&port=PORT&database=DATABASE&user=USER&password=PASSWORD" + "&recordSyntax=RECORDS_SYNTAX&charset=CHARSET&filePath=FILE_PATH" + "&recordIdPolicy=RECORD_ID_POLICY&idXpath=ID_XPATH" + "&namespacePrefix=NAMESPACE_PREFIX&namespaceUri=NAMESPACE_URI [Mandatory fields: " + "dataProviderId, id, description, schema, namespace, address, port, database, " + "recordSyntax, charset, filePath, recordIdPolicy (if " + "recordIdPolicy=IdExtracted the fields idXpath, namespacePrefix and namespaceUri " + "are mandatory)]");
                    }
                    ...
                    if (id != null && !id.isEmpty()) {
                        ((DefaultWebServices)webServices).updateDataSourceZ3950IdList(out, id, description, nameCode, name, exportPath, schema, namespace, address, port, database, user, password, recordSyntax, charset, xsdFile, recordIdPolicy, idXpath, namespacePrefix, namespaceUri);
                    } else {
                        webServices.createErrorMessage(out, MessageType.INVALID_REQUEST, "Error updating the" + "Z39.50 Data Source with ID list: invalid arguments." + "Syntax: /rest/dataSources/updateZ3950IdList?id=DATA_SOURCE_ID&description=DESCRIPTION" + "&nameCode=NAME_CODE&name=NAME&exportPath=EXPORT_PATH" + "&schema=SCHEMA&namespace=NAMESPACE" + "&address=ADDRESS&port=PORT&database=DATABASE&user=USER&password=PASSWORD" + "&recordSyntax=RECORDS_SYNTAX&charset=CHARSET&filePath=FILE_PATH" + "&recordIdPolicy=RECORD_ID_POLICY&idXpath=ID_XPATH" + "&namespacePrefix=NAMESPACE_PREFIX&namespaceUri=NAMESPACE_URI [Mandatory field: " + "id (if recordIdPolicy=IdExtracted the fields idXpath, namespacePrefix and namespaceUri " + "are mandatory)]");
                    }
                }
            } else {
    ...
    private boolean validateOperationParameter(RestRequest restRequest, OutputStream out, String operationParameter) throws IOException, DocumentException {
    ...
    private boolean validateDataSource(RestRequest restRequest, OutputStream out, String dataSourceId) throws IOException, DocumentException {
    ...
    private boolean validateRecordId(RestRequest restRequest, OutputStream out, String recordIdParameter) throws IOException, DocumentException {
    ...
    private boolean validateRecordUrn(RestRequest restRequest, ServletOutputStream out, String recordUrnString) throws IOException, DocumentException, SQLException {
        ...
        return !(!validateDataSource(restRequest, out, recordUrn.getDataSourceId()) || !validateRecordId(restRequest, out, recordUrnString) || ConfigSingleton.getRepoxContextUtil().getRepoxManager().getAccessPointsManager().getRecord(recordUrn) == null);

    }

    private boolean validateSaveParameters(RestRequest restRequest, OutputStream out, String dataSourceId, String operationParameter, String recordIdParameter) throws IOException, DocumentException {
        if (!validateOperationParameter(restRequest, out, operationParameter)) {
            RestUtils.writeInvalidRequest(restRequest.getFullRequestURI(), "operationParameter is mandatory for this operation: " + operationParameter, out);
            return false;
        } else if (!validateDataSource(restRequest, out, dataSourceId)) {
            RestUtils.writeInvalidRequest(restRequest.getFullRequestURI(), "dataSourceId parameter is mandatory for this operation: " + operationParameter, out);
            return false;
        } else {
            DataSource dataSource = repoxManager.getDataManager().getDataSourceContainer(dataSourceId).getDataSource();

            if (dataSource.getRecordIdPolicy() instanceof IdProvidedRecordIdPolicy && !validateRecordId(restRequest, out, recordIdParameter)) {
                RestUtils.writeInvalidRequest(restRequest.getFullRequestURI(), "recordId parameter is mandatory for this operation: " + operationParameter, out);
                return false;
            }
        }
